...
| `bun run check` | `build`, then `check:biome` (format and lint over `scripts`, `src` and the root JSON files), then `check:ts` (`tsc --noEmit` over everything `tsconfig.json` includes), then `check:leakage`. |
...
- Bun runs the scripts, but `tsc` is a `#!/usr/bin/env node` executable and runs under whatever Node is on PATH. The manifest's `engines` names 22 or later, and the pipeline installs exactly that; locally, an older Node fails in ways that read like a package defect.
