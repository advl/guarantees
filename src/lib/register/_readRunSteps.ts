...
/** A mapping the reader is inside, and whether a condition was hung on it. */
type _Scope = { readonly column: number; guarded: boolean };
 ...
 *
 * A condition is the same gesture one line shorter, so it is read too. An
 * `if:` on a step, or on the job the step is in, is a step that may run
 * nothing, and a command that may run nothing schedules nothing: the step's
 * commands are dropped, the tier they name looks unscheduled, and that is
 * already refused by name. Dropping rather than refusing outright is what
 * lets a job guard a step that names no tier at all, which is ordinary; the
 * cost falls only where the guarded step was the only thing naming a tier.
 *
...
export default function _readRunSteps(text: string): readonly string[] {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (scope === undefined || scope.column < column) {
      scope = { column, guarded: false };
      scopes.push(scope);
    }
    if (name === "if") scope.guarded = true;
